...
loop{
    ...
    Thread.start(server.accept) do |client|   #Thread.start is like Thread.new but allows a block to pass.

        request = client.read_nonblock(256)  #read_nonblock from I/O, with maxlen bytes
        ...
        if File.exist?(fileName)
            ...
            if header.include?('POST')
                ...
                user_data = "<li>name: #{params['client']['name']}</li>\n<li>e-mail: #{params['client']['email']}</li>"
